...
(function($){
	$.fn.extend({ 
		...
		var randID =  Math.round(Math.random()*100000000);
			...
			$(obj).prepend('<div id="dragwrapper'+randID+'"><div id="drag'+randID+'"><img width="70" height="38" alt="handle" src="'+o.imagePath+'handle.png" id="handle'+randID+'" /></div></div>'); // Create drag handle
			...
			// gray vertical line
			$('#drag'+randID).width(2).height(imgHeight).css({'background':o.dividerColor,'position':'absolute','left':'35px'});	// Set drag handle CSS properties
			$('#beforeimage'+randID).css({'position':'absolute','top':'0px','left':'0px'});
			$('#afterimage'+randID).css({'position':'absolute','top':'0px','right':'0px'});
			// handle png
			$('#handle'+randID).css({'z-index':'100','position':'relative','cursor':o.cursor,'top':(imgHeight/2)-($('#handle'+randID).height()/2)+'px','left':'-35px'})
			...
			{	
				...
				$('#showleft'+randID).css({'position':'relative','left':'35px'}).click(function(){
			...
			function drag()
			{
				...
				$('div:eq(2)', obj).width( parseInt( $(this).css('left') ) + 35 );
			}
			...
			function clickit()
			{
				...
				$(obj).one('mousemove', function(){$('#dragwrapper'+randID).stop();});
			}
  		});
    	}
	});
