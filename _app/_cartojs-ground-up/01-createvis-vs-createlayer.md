...
Looking back at our viz.json, we can see that the zeroth layer, buried under options, has a `layer_name` of "us_counties" and comes from our [us_counties dataset]({{ site.baseurl }}/d/counties.zip) back in the Beginner's Course. The second comes from another familiar [dataset]({{ site.baseurl }}/d/tornadoes.zip) on tornados in the United States. Other important info to pick out:
